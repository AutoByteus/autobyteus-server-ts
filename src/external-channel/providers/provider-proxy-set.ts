...
class DeferredChannelBindingProvider implements ChannelBindingProvider {
  ...
  async upsertBindingAgentId(bindingId: string, agentId: string): Promise<ChannelBinding> {
    return (await getProviderSet()).bindingProvider.upsertBindingAgentId(bindingId, agentId);
  }
...
class DeferredChannelMessageReceiptProvider implements ChannelMessageReceiptProvider {
  ...
  async getLatestSourceByAgentId(agentId: string): Promise<ChannelSourceContext | null> {
    return (await getProviderSet()).messageReceiptProvider.getLatestSourceByAgentId(agentId);
  }
  ...
  async getSourceByAgentTurn(
    ...
    return (await getProviderSet()).messageReceiptProvider.getSourceByAgentTurn(agentId, turnId);
  }
}
