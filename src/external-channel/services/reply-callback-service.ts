...
type ChannelSourceLookupPort = Pick<ChannelMessageReceiptService, "getSourceByAgentTurn">;
...
export class ReplyCallbackService {
  ...
  async publishAssistantReplyByTurn(
    ...
    const teamId = normalizeOptionalString(input.teamId ?? null);
    if (teamId) {
    ...
    const source = await this.messageReceiptService.getSourceByAgentTurn(agentId, turnId);
    if (!source) {
      return skip("SOURCE_NOT_FOUND");
    }

    const target: ChannelDispatchTarget = { agentId, teamId: null };
