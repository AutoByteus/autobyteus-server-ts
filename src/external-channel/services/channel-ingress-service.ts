...
export class ChannelIngressService {
  ...
  async handleInboundMessage(
    ...
    await this.messageReceiptService.recordIngressReceipt({
      ...
      agentId: normalizedDispatch.agentId,
      teamId: normalizedDispatch.teamId,
    });
...
const normalizeDispatchTarget = (
  ...
  const agentId = normalizeNullableString(dispatch.agentId) ?? binding.agentId;
  const teamId = normalizeNullableString(dispatch.teamId) ?? binding.teamId;
  if (!agentId && !teamId) {
    throw new Error(
      "Runtime dispatch must resolve to an agentId or teamId for receipt persistence.",
    );
  }
