...
export class ExternalChannelTurnReceiptBindingProcessor extends BaseAgentUserInputMessageProcessor {
  ...
  async process(
    ...
    if (!turnId) {
      logger.warn(
        `Agent '${context.agentId}': skipping external turn binding because activeTurnId is missing.`,
      );
      return message;
    }
    ...
    try {
      ...
      logger.error(
        `Agent '${context.agentId}': failed to bind external turn receipt: ${String(error)}`,
      );
    }
