...
type ArtifactServiceLike = {
  ...
  }) => Promise<{ id?: string | null; agentId: string; path: string; type: string; workspaceRoot?: string | null }>;
};
...
export class AgentArtifactPersistenceProcessor extends BaseToolExecutionResultProcessor {
  ...
  async process(event: ToolResultEvent, context: AgentContext): Promise<ToolResultEvent> {
    ...
    logger.debug(
      `AgentArtifactPersistenceProcessor: Processing tool '${event.toolName}' for agent '${agentId}'`,
    );
    ...
    try {
      ...
      if (outputPath) {
        ...
        if (artifactType) {
          ...
          logger.info(
            `Persisted generic ${artifactType} artifact for agent ${agentId} at ${outputPath}`,
          );
        }
      }
    } catch (error) {
  ...
  private notifyPersistence(
    ...
    artifact: { id?: string | null; agentId: string; path: string; type: string; workspaceRoot?: string | null },
    url: string | null,
  ): void {
