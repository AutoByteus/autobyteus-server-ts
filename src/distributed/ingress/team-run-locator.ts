...
export type TeamRunLocatorDependencies = {
  ...
  teamInstanceManager: Pick<
    AgentTeamInstanceManager,
    "getTeamInstance" | "getTeamDefinitionId" | "getTeamMemberNames"
  >;
  hostNodeId: string;
...
export class TeamRunLocator {
  ...
  private readonly teamInstanceManager: Pick<
    AgentTeamInstanceManager,
    "getTeamInstance" | "getTeamDefinitionId" | "getTeamMemberNames"
  >;
  ...
  constructor(deps: TeamRunLocatorDependencies) {
    ...
    this.teamInstanceManager = deps.teamInstanceManager;
  ...
  private assertTeamExists(teamId: string): void {
    const team = this.teamInstanceManager.getTeamInstance(teamId);
    if (!team) {
  ...
  private resolveTeamDefinitionIdOrThrow(teamId: string): string {
    const value = this.teamInstanceManager.getTeamDefinitionId(teamId);
    if (!value) {
  ...
  private assertTeamMembersExistInDefinition(teamId: string, definition: AgentTeamDefinition): void {
    const runtimeMemberNames = new Set(this.teamInstanceManager.getTeamMemberNames(teamId));
