...
export class AgentMemoryViewService {
  ...
  getAgentMemoryView(agentId: string, options: AgentMemoryViewOptions = {}): AgentMemoryView {
    ...
    if (includeWorkingContext) {
      const snapshotPayload = this.store.readWorkingContextSnapshot(agentId);
      workingContext = this.parseWorkingContext(snapshotPayload);
    }

    const episodic = includeEpisodic ? this.store.readEpisodic(agentId) : null;
    const semantic = includeSemantic ? this.store.readSemantic(agentId) : null;
    ...
    if (includeConversation || includeRawTraces) {
      const active = this.store.readRawTracesActive(agentId);
      const archive = includeArchive ? this.store.readRawTracesArchive(agentId) : [];
