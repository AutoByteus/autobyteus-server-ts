...
export class MemoryFileStore {
  ...
  getAgentDir(agentId: string): string {
    return path.join(this.baseDir, "agents", agentId);
  }
  ...
  readWorkingContextSnapshot(agentId: string): Record<string, unknown> | null {
    const filePath = path.join(this.getAgentDir(agentId), "working_context_snapshot.json");
    return this.readJson(filePath);
  }

  readRawTracesActive(agentId: string, limit?: number): Array<Record<string, unknown>> {
    const filePath = path.join(this.getAgentDir(agentId), "raw_traces.jsonl");
    return this.readJsonl(filePath, limit);
  }

  readRawTracesArchive(agentId: string, limit?: number): Array<Record<string, unknown>> {
    const filePath = path.join(this.getAgentDir(agentId), "raw_traces_archive.jsonl");
    return this.readJsonl(filePath, limit);
  }

  readEpisodic(agentId: string, limit?: number): Array<Record<string, unknown>> {
    const filePath = path.join(this.getAgentDir(agentId), "episodic.jsonl");
    return this.readJsonl(filePath, limit);
  }

  readSemantic(agentId: string, limit?: number): Array<Record<string, unknown>> {
    const filePath = path.join(this.getAgentDir(agentId), "semantic.jsonl");
    return this.readJsonl(filePath, limit);
  }
}
