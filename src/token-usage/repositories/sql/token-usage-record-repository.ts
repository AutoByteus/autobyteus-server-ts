...
export class SqlTokenUsageRecordRepository extends BaseRepository.forModel(
  ...
  async getUsageRecordsByAgentId(agentId: string): Promise<PrismaTokenUsageRecord[]> {
    try {
      ...
      logger.error(`Error retrieving usage records for agent ${agentId}: ${String(error)}`);
      throw error;
    }
  }
