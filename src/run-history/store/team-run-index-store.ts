...
export class TeamRunIndexStore {
  ...
  async getRow(teamId: string): Promise<TeamRunIndexRow | null> {
  ...
  async upsertRow(row: TeamRunIndexRow): Promise<void> {
    await this.enqueueWrite(async () => {
      ...
      const rows = index.rows.filter((entry) => entry.teamId !== row.teamId);
      rows.push(normalizeRow(row));
      await this.writeIndexDirect({
        version: TEAM_RUN_HISTORY_INDEX_VERSION,
        rows,
  ...
  async updateRow(
    teamId: string,
    patch: Partial<Omit<TeamRunIndexRow, "teamId">>,
  ): Promise<void> {
    await this.enqueueWrite(async () => {
      const index = await this.readIndexDirect();
      const current = index.rows.find((row) => row.teamId === teamId);
      if (!current) {
        return;
      }
  ...
  async removeRow(teamId: string): Promise<void> {
    await this.enqueueWrite(async () => {
      const index = await this.readIndexDirect();
      const rows = index.rows.filter((row) => row.teamId !== teamId);
