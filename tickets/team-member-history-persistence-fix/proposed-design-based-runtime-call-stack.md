...
1. `api/graphql/types/agent-team-instance.ts:sendMessageToTeam(input)`
...
5. `AgentTeamInstanceManager.createTeamInstanceWithId(teamId, teamDefinitionId, resolvedMemberConfigs)`.
...
## UC-2: Explicit createAgentTeamInstance
1. `api/graphql/types/agent-team-instance.ts:createAgentTeamInstance(input)`
2. Generate `teamId`.
3. Resolve member runtime configs with deterministic IDs.
4. Create runtime with `createTeamInstanceWithId(...)`.
...
4. `createTeamInstanceWithId(teamId, teamDefinitionId, memberConfigs)` without forcing divergent member memoryDir.
