...
1. `src/api/graphql/types/agent-team-instance.ts:sendMessageToTeam(...)`
2. Branch: `teamId` absent -> lazy create path.
3. `AgentTeamInstanceManager.createTeamInstance(teamDefinitionId, memberConfigs)`
4. `AgentTeamInstanceResolver.resolveTeamDefinitionMetadata(teamDefinitionId)`
5. `AgentTeamDefinitionService.getDefinitionById(teamDefinitionId)`
6. `AgentTeamInstanceResolver.buildTeamRunManifest({ teamId, teamDefinitionId, teamDefinitionName, coordinatorMemberName, memberConfigs })`
...
1. `src/api/graphql/types/agent-team-instance.ts:createAgentTeamInstance(...)`
2. `AgentTeamInstanceManager.createTeamInstance(...)`
3. `AgentTeamInstanceResolver.resolveTeamDefinitionMetadata(teamDefinitionId)`
4. `AgentTeamDefinitionService.getDefinitionById(teamDefinitionId)`
5. `AgentTeamInstanceResolver.buildTeamRunManifest(...)`
