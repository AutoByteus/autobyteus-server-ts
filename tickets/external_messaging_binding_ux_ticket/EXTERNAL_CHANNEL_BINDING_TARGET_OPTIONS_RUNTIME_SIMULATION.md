...
└── src/external-channel/services/channel-binding-target-options-service.ts:listActiveTargetOptions() [ASYNC]
    ├── src/agent-execution/services/agent-instance-manager.ts:listActiveInstances() [STATE]
    ├── src/agent-execution/services/agent-instance-manager.ts:getAgentInstance(id) [STATE]
...
└── channel-binding-target-options-service.ts:listActiveTargetOptions() [ASYNC]
    ├── src/agent-team-execution/services/agent-team-instance-manager.ts:listActiveInstances() [STATE]
    ├── src/agent-team-execution/services/agent-team-instance-manager.ts:getTeamInstance(id) [STATE]
