...
1. Web calls `sendAgentUserInput` (`src/api/graphql/types/agent-instance.ts`)
2. Server builds agent with selected definition + model (`src/agent-execution/services/agent-instance-manager.ts`)
...
2. `src/api/graphql/types/agent-instance.ts:sendAgentUserInput(...)`
3. `src/agent-execution/services/agent-instance-manager.ts:createAgentInstance(...)`
