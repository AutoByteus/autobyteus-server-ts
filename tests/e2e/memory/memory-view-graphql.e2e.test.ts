...
describe("Memory view GraphQL e2e", () => {
  ...
  it("returns memory view with conversation and raw traces", async () => {
    ...
    const query = `
      query MemoryView($agentId: String!) {
        getAgentMemoryView(agentId: $agentId, includeRawTraces: true, includeArchive: true) {
          agentId
    ...
    const data = await execGraphql<{ getAgentMemoryView: { agentId: string; workingContext: Array<{ role: string }>; conversation: Array<{ kind: string }>; rawTraces: Array<{ traceType: string }> } }>(
      query,
      { agentId },
    );

    expect(data.getAgentMemoryView.agentId).toBe(agentId);
    expect(data.getAgentMemoryView.workingContext[0]?.role).toBe("user");
    expect(data.getAgentMemoryView.conversation[0]?.kind).toBe("message");
    expect(data.getAgentMemoryView.rawTraces.length).toBeGreaterThan(0);
  });
});
