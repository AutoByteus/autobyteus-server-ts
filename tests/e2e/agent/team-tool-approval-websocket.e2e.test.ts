...
describe("Team tool-approval websocket e2e", () => {
  ...
  it("routes approval requests and approvals across websocket boundary using canonical member target", async () => {
    ...
    const getTeamInstanceSpy = vi
      .spyOn(teamManager, "getTeamInstance")
      .mockImplementation((requestedTeamId: string) =>
        requestedTeamId === teamId ? ({ teamId } as unknown as ReturnType<typeof teamManager.getTeamInstance>) : null,
      );
    ...
    expect(getTeamInstanceSpy).toHaveBeenCalledWith(teamId);
