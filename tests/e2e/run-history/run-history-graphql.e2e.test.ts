...
describe("Run history GraphQL e2e", () => {
  ...
  it("terminates an active run and marks history as inactive", async () => {
    ...
    const manager = AgentInstanceManager.getInstance();
    let isActive = true;

    vi.spyOn(manager, "getAgentInstance").mockImplementation((agentId: string) => {
    ...
    vi.spyOn(manager, "listActiveInstances").mockImplementation(() =>
      isActive ? [runId] : [],
    );
    vi.spyOn(manager, "terminateAgentInstance").mockImplementation(async (agentId: string) => {
      if (agentId !== runId) {
        return false;
      }
      isActive = false;
    ...
    expect(terminateResult.terminateAgentInstance.success).toBe(true);
    expect(terminateResult.terminateAgentInstance.message).toContain("terminated");
  ...
  it("restores an inactive run through continueRun and marks it active", async () => {
    ...
    vi.spyOn(manager, "restoreAgentInstance").mockImplementation(async (options: any) => {
      if (options.agentId === runId) {
        restored = true;
      }
