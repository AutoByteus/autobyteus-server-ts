...
describe("PromptSyncService helpers", () => {
    it("parses suitable_for_models into a set", () => {
        ...
        expect(service.parseSuitableForModels("Claude 3.7, ChatGPT-4001, 01-mini")).toEqual(new Set(["Claude 3.7", "ChatGPT-4001", "01-mini"]));
        expect(service.parseSuitableForModels(" Claude 3.7 , ChatGPT-4001 ")).toEqual(new Set(["Claude 3.7", "ChatGPT-4001"]));
        expect(service.parseSuitableForModels("Claude 3.7,,ChatGPT-4001")).toEqual(new Set(["Claude 3.7", "ChatGPT-4001"]));
    });
...
describe("PromptSyncService sync logic (DB + mocked fetch)", () => {
    ...
    it("creates a new prompt when model lists are empty", async () => {
        ...
        const initialPrompts = await promptService.findAllByNameAndCategory(name, "Test Category");
...
describe("PromptSyncService end-to-end", () => {
    const shouldRun = process.env.AUTOBYTEUS_MARKETPLACE_HOST === "http://localhost:8020" &&
