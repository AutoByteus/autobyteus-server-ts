...
describe("SqlAgentConversationMessageRepository", () => {
    ...
    it("creates messages", async () => {
        ...
        const result = await messageRepo.createMessage(PrismaConversationConverter.toMessageCreateInput(messageData));
    ...
    it("gets messages by conversation id", async () => {
        const conversation = await createConversation();
        await messageRepo.createMessage(PrismaConversationConverter.toMessageCreateInput({
            agentConversationId: conversation.id,
            role: "user",
            message: "msg1",
        }));
    ...
    it("updates token usage", async () => {
        const conversation = await createConversation();
        const msg = await messageRepo.createMessage(PrismaConversationConverter.toMessageCreateInput({
            agentConversationId: conversation.id,
            role: "user",
            message: "msg",
        }));
