...
describe("AgentTeamInstanceManager restore-after-terminate integration", () => {
  ...
  it("recreates same teamId and memberAgentId after terminate", async () => {
    ...
    await manager.createTeamInstanceWithId(teamId, teamDefinitionId, memberConfigs);
    await manager.terminateTeamInstance(teamId);

    await expect(
      manager.createTeamInstanceWithId(teamId, teamDefinitionId, memberConfigs),
