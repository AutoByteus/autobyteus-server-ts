...
const buildDomainDefinition = () => new AgentWorkflowDefinition({
    id: "123",
    name: "Domain Workflow",
    description: "A test workflow object.",
    nodes: sampleNodesData.map((node) => new WorkflowNode({
        nodeId: node.node_id,
...
describe("PrismaWorkflowDefinitionConverter", () => {
    ...
    it("converts domain to Prisma create input", () => {
        ...
        const nodesPayload = JSON.parse(createInput.nodes);
    ...
    it("throws when updating without an id", () => {
        const domainObj = buildDomainDefinition();
        domainObj.id = null;
        expect(() => PrismaWorkflowDefinitionConverter.toUpdateInput(domainObj)).toThrow("AgentWorkflowDefinition id is required for update");
    });
});
