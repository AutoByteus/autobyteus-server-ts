...
import { WorkflowDefinitionService, } from "../../../src/workflow-definition/services/workflow-definition-service.js";
...
describe("WorkflowDefinitionService", () => {
    ...
    const buildDomainDefinition = (id) => {
        const input = buildCreateInput();
        return new AgentWorkflowDefinition({
            id,
            name: input.name,
            description: input.description,
            nodes: input.nodes.map((node) => new WorkflowNode({
                nodeId: node.nodeId,
    ...
    beforeEach(() => {
        provider = {
            create: vi.fn(async (definition) => new AgentWorkflowDefinition({
                id: "def-123",
    ...
    it("throws when updating missing definitions", async () => {
        const service = buildService();
        provider.getById.mockResolvedValue(null);
        await expect(service.updateDefinition("missing-id", { description: "Updated" })).rejects.toThrow("Workflow Definition with ID missing-id not found.");
    });
    ...
    it("throws when deleting missing definitions", async () => {
        const service = buildService();
        provider.getById.mockResolvedValue(null);
        await expect(service.deleteDefinition("missing-id")).rejects.toThrow("Workflow Definition with ID missing-id not found.");
    });
});
