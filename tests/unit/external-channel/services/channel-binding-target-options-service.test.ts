...
describe("ChannelBindingTargetOptionsService", () => {
  it("lists active target options, filters stale entries, and sorts output", async () => {
    const service = new ChannelBindingTargetOptionsService({
      agentManager: {
        listActiveInstances: () => ["agent-2", "agent-1", "agent-stale"],
        getAgentInstance: (id: string) => {
          if (id === "agent-1") {
            return {
              agentId: "agent-1",
      ...
      teamManager: {
        listActiveInstances: () => ["team-1"],
        getTeamInstance: (id: string) =>
          id === "team-1"
            ? {
                teamId: "team-1",
                name: "Primary Team",
  ...
  it("validates whether selected target is active", async () => {
    const service = new ChannelBindingTargetOptionsService({
      agentManager: {
        listActiveInstances: () => ["agent-1"],
        getAgentInstance: (id: string) => (id === "agent-1" ? { agentId: id } : null),
      },
      teamManager: {
        listActiveInstances: () => ["team-1"],
        getTeamInstance: (id: string) => (id === "team-1" ? { teamId: id } : null),
      },
    });
