...
describe("TeamRunHistoryService", () => {
  ...
  let fakeTeamManager: { getTeamInstance: ReturnType<typeof vi.fn> };
  ...
  beforeEach(async () => {
    ...
    service = new TeamRunHistoryService(memoryDir, {
      teamInstanceManager: fakeTeamManager as any,
    });
  ...
  it("upserts row and reports active status from runtime manager", async () => {
    fakeTeamManager.getTeamInstance.mockImplementation((teamId: string) =>
      teamId === "team-1" ? ({ teamId } as any) : null,
    );
  ...
  it("returns resume config and delete safety checks", async () => {
    ...
    expect(resume.teamId).toBe("team-3");
    expect(resume.isActive).toBe(false);

    fakeTeamManager.getTeamInstance.mockReturnValue({ teamId: "team-3" } as any);
    ...
    fakeTeamManager.getTeamInstance.mockReturnValue(null);
