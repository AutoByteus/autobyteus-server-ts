...
class MockLLM extends BaseLLM {
    ...
    async _sendUserMessageToLLM(_userMessage, _kwargs) {
        return new CompleteResponse({ content: "mock response" });
    }
    async *_streamUserMessageToLLM(_userMessage, _kwargs) {
...
const buildAgentContext = () => {
    ...
    const config = new AgentConfig("test_agent", "tester", "A test agent", new MockLLM("test-llm-v1"), "You are a test agent.", [], true, null, null, null, null, null, null, null, initialData);
...
describe("CreateAgentConversationRecordProcessor", () => {
    ...
    it("processes successfully and creates conversation record", async () => {
        ...
        expect(mockPersistenceProxy.createConversation).toHaveBeenCalledWith("agent_xyz_789", "test_definition_123", "test-llm-v1", false);
    });
    it("logs errors when persistence fails", async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("CRITICAL - Failed to create persistence record"));
