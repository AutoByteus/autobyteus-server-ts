...
describe("UserInputPersistenceProcessor", () => {
  it("persists user input with context files", async () => {
    ...
    const context = { agentId: "agent_abc_789" } as AgentContext;

    const inputToProcess = new AgentInputUserMessage("This might be a modified message.");
    const originalInput = new AgentInputUserMessage(
      "This is the original user requirement.",
      undefined,
      [new ContextFile("src/main.py"), new ContextFile("/tmp/data.csv")],
    );
