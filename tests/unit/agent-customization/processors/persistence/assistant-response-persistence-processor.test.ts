...
describe("AssistantResponsePersistenceProcessor", () => {
  it("persists response with token usage", async () => {
    ...
    const context = { agentId: "agent_abc_123" } as AgentContext;
  ...
  it("logs when persistence fails", async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining("Failed to persist assistant response"),
    );
  ...
  it("exposes name", () => {
    expect(AssistantResponsePersistenceProcessor.getName()).toBe(
